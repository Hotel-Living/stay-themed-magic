...
export function HotelCard({
  ...
  const displayThemes = themes.length > 0 
    ? themes 
    : hotel_themes
        ?.filter(ht => ht.themes?.name)
        .map(ht => ({ id: ht.themes!.name, name: ht.themes!.name })) || [];

  return (
    <Card 
      className="overflow-hidden hover:shadow-lg transition-all duration-300 cursor-pointer group bg-gradient-to-br from-purple-900/20 to-indigo-900/20 border border-purple-500/30 backdrop-blur-sm"
      onClick={onClick}
    >
