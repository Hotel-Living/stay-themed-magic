...
export const HotelCard = ({
  ...
  return (
    <Card 
      className="overflow-hidden transition-transform hover:scale-[1.02] cursor-pointer glass-card bg-gradient-to-b from-purple-900/90 to-purple-800/80 backdrop-blur-md border border-purple-600/30 rounded-2xl shadow-2xl w-[90%]"
      onClick={onClick}
    >
          ...
          <div className="text-center space-y-3">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {displayThemes.slice(0, 4).map((theme, index) => (
                <span 
                  key={theme}
                  className={`px-3 py-1 rounded-full text-xs font-medium border shadow-sm ${tagColors[index % tagColors.length]}`}
                >
                  {theme}
                </span>
              ))}
          ...
          <div className="text-center space-y-3">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {displayActivities.slice(0, 4).map((activity, index) => (
                <span 
                  key={activity}
                  className={`px-3 py-1 rounded-full text-xs font-medium border shadow-sm ${tagColors[(index + 4) % tagColors.length]}`}
                >
                  {activity}
                </span>
              ))}
