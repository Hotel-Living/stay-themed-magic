  ...
  return (
    <div ref={bgRef} className="background-slideshow fixed inset-0 -z-10" style={{ backgroundColor: "#C9CAFE" }}>
      {backgroundImages.map((image, index) => (
        <div 
          key={index}
          className={`bg-slide absolute inset-0 bg-cover bg-center transition-opacity duration-3000 ease-in-out ${currentSlide === index ? 'opacity-95' : 'opacity-0'}`}
          style={{ backgroundImage: `url(${image})` }}
        />
      ))}
      <div className="overlay absolute inset-0 bg-black/9 backdrop-blur-[2.1px]" />
    </div>
  );
}
