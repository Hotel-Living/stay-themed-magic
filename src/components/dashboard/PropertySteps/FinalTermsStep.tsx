  ...
  return <div className="space-y-6">
      ...
      <div className="bg-fuchsia-900/10 rounded-lg p-6">
        ...
        <div className="space-y-4">
          {faqItems.map(item => <div key={item.id} className="bg-fuchsia-950/30 rounded-lg p-4 space-y-3">
              <div className="flex justify-between items-start">
                <div className="flex-1 space-y-3">
                  <Input placeholder={t('dashboard-faq-terms.questionPlaceholder')} value={item.question} onChange={e => updateFaqItem(item.id, 'question', e.target.value)} className="bg-fuchsia-950/50 border-fuchsia-500/30 text-white" />
                  <Textarea placeholder={t('dashboard-faq-terms.answerPlaceholder')} value={item.answer} onChange={e => updateFaqItem(item.id, 'answer', e.target.value)} className="bg-fuchsia-950/50 border-fuchsia-500/30 text-white h-20" />
                </div>
                <Button variant="ghost" size="sm" onClick={() => removeFaqItem(item.id)} className="ml-2 text-red-400 hover:text-red-300">
                  <Trash2 className="h-4 w-4" />
                </Button>
              </div>
            </div>)}
          
          <Button onClick={addFaqItem} variant="outline" className="w-full border-fuchsia-500/50 text-fuchsia-300 hover:bg-fuchsia-500/10">
      ...
      <div className="bg-fuchsia-900/10 rounded-lg p-6">
        ...
        <div className="flex items-start space-x-3">
          <input type="checkbox" id="terms-accepted" checked={formData?.termsAccepted || false} onChange={e => updateFormData("termsAccepted", e.target.checked)} className="w-5 h-5 mt-1" />
