      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="bg-fuchsia-900/50">
            ...
            {stayDurations.map(duration => <th key={duration} className="p-2 text-center text-xs bg-[#034503]">{duration} DAYS</th>)}
          </tr>
        </thead>
        <tbody>
          {mealTypes.map((mealType, index) => <tr key={mealType} className={index % 2 === 0 ? "bg-fuchsia-900/20" : "bg-fuchsia-900/30"}>
              <td className="p-2 text-xs font-medium bg-[#034e03]">{mealType}</td>
              {stayDurations.map(duration => <td key={`${mealType}-${duration}`} className="p-2 text-center text-xs">
                  <input type="number" placeholder="0.00" required className="w-16 border border-fuchsia-800/30 rounded-lg p-1 text-center bg-[#014f04]" />
                </td>)}
            </tr>)}
        </tbody>
      </table>
