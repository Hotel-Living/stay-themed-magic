...
export default function StepFour({
  ...
  return <div className="space-y-6">
      ...
      <div className="mt-8">
        ...
        {selectedThemes.length > 0 && <div className="flex flex-wrap gap-2 mb-4">
            {selectedThemes.map(theme => <div key={theme} className="px-3 py-1 rounded-full bg-fuchsia-100 text-fuchsia-800 flex items-center gap-1">
                <span>{theme}</span>
                <button onClick={() => setSelectedThemes(selectedThemes.filter(t => t !== theme))} className="w-4 h-4 rounded-full flex items-center justify-center hover:bg-fuchsia-200">
                  <X className="w-3 h-3" />
                </button>
              </div>)}
          </div>}
        ...
        {selectedThemes.length > 0 && !error && <div className="p-3 rounded-md bg-green-50 text-green-700 flex items-center gap-2">
