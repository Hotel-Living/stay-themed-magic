...
export default function ThemesAndActivitiesStep({
  ...
  return <div className="space-y-4">
      ...
      <Link to="/themes-information" className="inline-flex items-center px-4 py-2 rounded-lg text-white text-sm font-medium transition-colors mb-4 bg-[#e108fd]/80 hover:bg-[#e108fd]">
        More Information
      </Link>
      
      <div>
        <div className="grid grid-cols-1 gap-0.5">
          {sortedThemeCategories.map(category => <ThemeCategory key={category.category} category={category} isOpen={openCategory === category.category} toggleCategory={toggleCategory} openSubmenus={{
          [openSubmenu || '']: !!openSubmenu
        }} toggleSubmenu={toggleSubmenu} onThemeSelect={handleThemeSelection} />)}
        </div>
      </div>

      {selectedThemes.length === 0 && <p className="text-sm mt-2 text-[#fa97a3]/[0.99]">
