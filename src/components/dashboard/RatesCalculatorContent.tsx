        ...
        <Tabs value={mainTab} onValueChange={() => {}} className="w-full">
          ...
          {modelExpanded && (
            <TabsContent value="model-rates-calculator">
              ...
              <div className="mb-4 w-full transition-all duration-300">
                {/* TIPS & STRATEGIC GUIDELINES menu (now as Profits-style full tabs) */}
                <div className="w-full">
                  <Tabs
                    value={openTipsSubmenu || ""}
                    onValueChange={setOpenTipsSubmenu}
                    className="w-full"
                  >
                    <TabsList className="grid w-full grid-cols-4 mb-4 rounded-xl overflow-hidden p-1 bg-gradient-to-br from-cyan-400 to-purple-600 shadow-lg">
                      <TabsTrigger
                        value="general"
                        className="text-xs md:text-base font-bold uppercase tracking-tight flex-1 rounded-none data-[state=active]:bg-[#12002b] data-[state=active]:text-fuchsia-100 text-white bg-transparent px-1 md:px-4 py-4 transition-all"
                        style={{ minWidth: 0 }}
                      >
              ...
              {/* DOWNLOAD ONLINE CALCULATOR section now is always below TIPS tabs and NOT part of the split layout */}
              {/* remove logic that previously placed download section beside TIPS when open */}
              <div
