...
const hotelRegistrationSchema = z.object({
  ...
  // Property details
  propertyType: z.enum(['Hotel', 'Boutique Hotel', 'Resort', 'Rural House', 'Hostel'], {
  ...
  checkInDay: z.enum(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'], {
    required_error: 'Check-in day is required'
  }),
  mealPlan: z.enum(['Room only', 'Breakfast', 'Half board', 'Full board', 'All-inclusive'], {
  ...
  const onSubmit = async (data: HotelRegistrationFormData) => {
    ...
    try {
      ...
      const hotelData = {
        ...
        category: parseInt(data.classification.length.toString()),
        ...
        features_hotel: data.hotelFeatures.reduce((acc, feature) => ({
          ...acc,
          [feature]: true
        }), {}),
        features_room: data.roomFeatures?.reduce((acc, feature) => ({
          ...acc,
          [feature]: true
        }), {}) || {},
        room_types: [{
      ...
      const {
        data: result,
        error
      } = await supabase.from('hotels').insert([hotelData]).select().single();
      if (error) {
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="pt-6 border-t border-white/10">
            <button type="submit" disabled={isSubmitting} className="w-full bg-gradient-to-r from-fuchsia-500 to-purple-600 hover:from-fuchsia-600 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold py-4 px-6 rounded-lg transition-all duration-200">
