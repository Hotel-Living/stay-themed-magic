        ...
        {faqCategories.map(category => {
          ...
          return (
            <TabsTrigger 
              key={category.id} 
              value={category.id} 
              className={`${isMobile ? "px-2 py-2 text-[8px]" : "px-2 sm:px-5 py-2 sm:py-4 text-[10px] sm:text-xs"} uppercase whitespace-pre-line text-center bg-transparent data-[state=active]:bg-transparent data-[state=active]:text-white text-white hover:bg-transparent hover:text-white transition-all duration-200 rounded-none font-bold tracking-wide ${!isMobile ? "flex-shrink-0" : ""} min-w-0 border-none`}
