      ...
      <div className="container relative z-10 max-w-6xl mx-auto text-center">
        <div className={`pt-4 ${isMobile ? "mb-6" : "mb-0.5"}`}>
          ...
          <div className={`flex justify-center ${isMobile ? "mb-8" : "mb-1"}`}>
            <div className={`relative ${isMobile ? "w-[75%]" : "w-[30%]"} mx-auto`}>
              <div className="rounded-lg px-2 py-1 border-2 border-[#D946EF] bg-black/30 backdrop-blur-sm shadow-[0_0_15px_rgba(217,70,239,0.7)]">
                <p className={`${isMobile ? "text-2xl" : "text-lg sm:text-xl md:text-2xl"} font-bold animate-pulse-glow max-w-full text-center`}
                   style={{
                     color: "#fff",
          ...
          <h1 className={`${isMobile ? "text-5xl" : "text-4xl sm:text-5xl md:text-6xl lg:text-7xl"} font-bold mb-0.25 text-white drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]`}>
        ...
        <div className={`max-w-3xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-0.25 ${isMobile ? "mb-6 mt-6 px-0" : "mb-0.5"} bg-white/15 rounded-lg backdrop-blur-sm p-2`}>
          <div className={`${isMobile ? "space-y-3 pl-0" : "space-y-0.125"}`}>
            {["Get rid of household chores!", "Select hotels upon favourite themes!"].map((slogan, index) => <div key={index} className={`flex items-center ${isMobile ? "gap-2 pl-1" : "gap-2"} p-0.5 rounded-lg`}>
                ...
                <p className={`text-left font-bold ${isMobile ? "text-lg" : "text-sm sm:text-lg"} text-white drop-shadow-[0_1px_1px_rgba(0,0,0,0.7)]`}>
                  {slogan}
                </p>
              </div>)}
          </div>
