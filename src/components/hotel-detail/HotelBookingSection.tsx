...
import { formatCurrency, calculateDynamicPrice, calculateTotalNightsInMonth, calculateNightsSold } from "@/utils/dynamicPricing";
...
export function HotelBookingSection({
  ...
  const getPriceForDuration = (duration: number) => {
    ...
    if (pricingMatrix && pricingMatrix.length > 0) {
      ...
      const matrixEntry = pricingMatrix.find(entry => entry.roomType === selectedRoomType && entry.stayLength === stayLengthStr && entry.mealPlan === selectedMealPlan);
    ...
    if (!basePrice && rates) {
      ...
      const simplePossibleKeys = [duration.toString(), duration, `${duration}`, `price_${duration}`, `${duration}_days`];
  ...
  const getDisplayPrice = () => {
    ...
    const finalPrice = calculateDynamicPrice(basePrice, totalNights, soldNights, priceIncreaseCap);
  ...
  return <div className="bg-fuchsia-950/30 rounded-xl p-4 shadow">
      ...
      <div className="mb-4 text-center">
        ...
        if (price) {
          return <div key={duration} className="text-sm text-white mb-1">
                {duration} nights – {formatCurrency(price, currency)}
              </div>;
        }
        return null;
      }).filter(Boolean).length > 0 ? null : <div className="text-sm text-white/70 mb-1">
      ...
      <Calendar className="mb-4" selected={checkInDate} onSelect={setCheckInDate} disabled={isDateDisabled} />

      <h2 className="text-lg font-semibold mb-2 text-white">{getStayDurationLabel()}</h2>
      <div className="flex flex-wrap gap-2 mb-2">
        {stayDurations.map(d => <Button variant={selectedDuration === d ? "default" : "outline"} key={d} onClick={() => setSelectedDuration(d)} className={cn("border-white transition-colors", selectedDuration === d ? "bg-[#8000B0] text-white hover:bg-[#5A0080]" : "bg-[#8000B0] text-white hover:bg-[#5A0080] border-[#8000B0]")}>
            {d}
          </Button>)}
      </div>
      ...
      <Button className="mt-3 w-full bg-white text-[#000066] hover:bg-white/90" onClick={handleBookClick}>
        Book
      </Button>
