    ...
    const mealPlanDisplayNames = mealPlans.map(plan => {
      switch (plan) {
        case 'breakfast-included':
          return t('hotels.mealPlans.breakfast');
        case 'half-board':
          return t('hotels.mealPlans.halfBoard');
        case 'full-board':
        case 'fullBoard':
          return t('hotels.mealPlans.fullBoard');
        case 'all-inclusive':
          return t('hotels.mealPlans.allInclusive');
        case 'laundry':
          return t('hotels.mealPlans.laundry');
        case 'external-laundry':
          return t('hotels.mealPlans.externalLaundry');
        default:
  ...
  return <Card className="bg-[#6000B3] border-border shadow-2xl">
      <div className="p-6 space-y-6 bg-[#6000B3]">
        <div className="text-center">
          <h2 className="text-2xl font-bold mb-2 text-white">{t('hotels.bookYourStay')}</h2>
          <p className="text-white/80 mb-2">
            {t('hotels.weeklyCheckInDay')}: {preferredWeekday}
          </p>
          
          {/* Display meal plans if available */}
          {mealPlans && mealPlans.length > 0 && <p className="text-white/90 mb-4 text-sm">
              {t('hotels.meals')}: {formatMealPlans()}
            </p>}
        </div>

        {/* Room Type Selection */}
        {roomTypes.length > 0 && <div className="space-y-2">
            <label className="text-sm font-semibold text-white">{t('hotels.roomType')}</label>
            <Select value={selectedRoomType} onValueChange={setSelectedRoomType}>
              <SelectTrigger className="bg-[#6000B3]/30 border-border text-white">
                <SelectValue placeholder={t('hotels.selectRoomType')} />
              </SelectTrigger>
        ...
        {stayDurations.length > 0 && <div className="space-y-2">
            <label className="text-sm font-semibold text-white">{t('hotels.stayDuration')}</label>
            <Select value={selectedDuration.toString()} onValueChange={value => setSelectedDuration(parseInt(value))}>
              ...
              <SelectContent className="bg-[#6000B3] border-border z-50">
                {stayDurations.map(duration => <SelectItem key={duration} value={duration.toString()} className="text-white font-bold hover:bg-white/10">
                    {duration} {duration === 1 ? t('hotels.day') : t('hotels.days')}
                  </SelectItem>)}
              </SelectContent>
            </Select>
          </div>}
        ...
        <div className="space-y-2">
          <label className="text-sm font-semibold text-white">{t('hotels.checkInDate')}</label>
        ...
        {checkInDate && checkoutDate && <div className="bg-[#6000B3]/30 rounded-lg p-4 border border-border">
            <div className="text-center">
              <h3 className="text-lg font-semibold text-white mb-2">{t('hotels.yourStayDetails')}</h3>
              <div className="space-y-1 text-white/90">
                <p><strong>{t('hotels.checkIn')}:</strong> {format(checkInDate, "EEEE, MMMM do, yyyy")}</p>
                <p><strong>{t('hotels.checkOut')}:</strong> {format(checkoutDate, "EEEE, MMMM do, yyyy")}</p>
                <p className="bg-[#6000B3]"><strong>{t('hotels.duration')}:</strong> {selectedDuration} {selectedDuration === 1 ? t('hotels.day') : t('hotels.days')}</p>
                {selectedRoomType && <p><strong>{t('hotels.roomType')}:</strong> {selectedRoomType}</p>}
              </div>
            </div>
          </div>}
          ...
          <div className="text-center">
            {currentPrice !== null ? <>
                <p className="text-white/80 text-sm font-bold">
                  {isPricePerPerson ? t('hotels.pricePerPerson') : t('hotels.priceForRoom')}
                </p>
                <p className="text-2xl font-bold text-white">
                  {currency === 'USD' ? '$' : currency === 'EUR' ? '€' : currency}{currentPrice.toLocaleString()}
                </p>
                <p className="text-white/70 text-xs font-bold">{t('hotels.for')} {selectedDuration} {selectedDuration === 1 ? t('hotels.day') : t('hotels.days')}</p>
                {isPricePerPerson && <p className="text-yellow-200/80 text-xs mt-1 font-bold">
                    {t('hotels.totalForTwoGuests')}: {currency === 'USD' ? '$' : currency === 'EUR' ? '€' : currency}{(currentPrice * 2).toLocaleString()}
                  </p>}
              </> : <>
                <p className="text-white/80 text-sm font-bold">{t('hotels.pricingInformation')}</p>
                <p className="text-lg font-semibold text-white bg-[#6000B3]">{t('hotels.priceNotAvailable')}</p>
                <p className="text-white/70 text-xs font-bold">{t('hotels.contactForRates')}</p>
              </>}
          </div>
        </div>
        ...
        <div className="bg-gradient-to-r from-[#6000B3]/40 to-[#6000B3]/30 border border-border rounded-lg p-4">
          <div className="text-center">
            <p className="text-white text-sm flex items-center justify-center gap-2 font-bold">
              🔔 <strong>{t('hotels.takeAdvantageOfPrice')}</strong>
            </p>
            <p className="text-white/80 text-xs mt-1 font-bold">
              {t('hotels.dynamicPricingMessage')}
            </p>
          </div>
        </div>
