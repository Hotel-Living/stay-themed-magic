...
export function JoinUsForm() {
  ...
  return <Section icon={Star} title="Want to Join Us?">
      ...
      <ContactForm renderFileUpload={() => <div className="mb-6">
            <label htmlFor="resume-upload" className={`flex items-center gap-2 px-4 py-2 ${files.length >= 5 ? 'bg-gray-600 cursor-not-allowed' : 'bg-[#8017B0]/80 hover:bg-[#8017B0] cursor-pointer'} text-white rounded-md w-fit transition-colors`}>
              {isUploading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Upload className="w-4 h-4" />}
              
              <input id="resume-upload" type="file" multiple className="hidden" onChange={handleFileChange} disabled={files.length >= 5 || isUploading} accept=".pdf,.doc,.docx,.txt,.jpg,.jpeg,.png" />
            </label>
          ...
          </div>} files={files} recipientEmail="grand_soiree@yahoo.com" onSubmitSuccess={handleFormSubmitted} />
    </Section>;
}