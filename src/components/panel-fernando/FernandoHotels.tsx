...
import { Plus, Search, Download, Edit, Trash2, Check, X } from "lucide-react";
...
export default function FernandoHotels() {
  ...
  const handleApproveHotel = async (hotelId: string) => {
    if (processingActions.has(hotelId)) return;
    
    setProcessingActions(prev => new Set(prev).add(hotelId));
  ...
  const handleRejectHotel = async (hotelId: string) => {
    ...
    try {
      console.log('Rejecting hotel:', hotelId);
      const { error } = await supabase
        .from('hotels')
                        ...
                        {hotel.status === 'pending' && (
                          <>
                            <Button
                              ...
                              disabled={processingActions.has(hotel.id)}
                              title="Approve Hotel"
                            >
