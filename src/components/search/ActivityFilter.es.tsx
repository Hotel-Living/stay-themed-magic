...
export function ActivityFilterES({ activeActivities, onChange }: ActivityFilterESProps) {
  ...
  if (loading) {
    return (
      <FilterItem title="ACTIVIDADES">
        <div className="text-sm text-fuchsia-300/70 px-3 py-2">Cargando actividades...</div>
      </FilterItem>
    );
  }
  ...
  if (error || activities.length === 0) {
    return (
      <FilterItem title="ACTIVIDADES">
        <div className="text-sm text-fuchsia-300/70 px-3 py-2">No hay actividades disponibles</div>
      </FilterItem>
    );
  }
