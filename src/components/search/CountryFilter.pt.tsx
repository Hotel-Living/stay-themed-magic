...
export function CountryFilterPT({ activeCountry, onChange }: CountryFilterPTProps) {
  ...
  if (loading) {
    return (
      <FilterItem title="PAÍS">
        <div className="text-sm text-fuchsia-300/70 px-3 py-2">Carregando países...</div>
      </FilterItem>
    );
  }
  ...
  if (error || countries.length === 0) {
    return (
      <FilterItem title="PAÍS">
        <div className="text-sm text-fuchsia-300/70 px-3 py-2">Nenhum país disponível</div>
      </FilterItem>
    );
  }
