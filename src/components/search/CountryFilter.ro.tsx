...
export function CountryFilterRO({ activeCountry, onChange }: CountryFilterROProps) {
  ...
  if (loading) {
    return (
      <FilterItem title="ȚARĂ">
        <div className="text-sm text-fuchsia-300/70 px-3 py-2">Se încarcă țările...</div>
      </FilterItem>
    );
  }
  ...
  if (error || countries.length === 0) {
    return (
      <FilterItem title="ȚARĂ">
        <div className="text-sm text-fuchsia-300/70 px-3 py-2">Nu sunt țări disponibile</div>
      </FilterItem>
    );
  }
