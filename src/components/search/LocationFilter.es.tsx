...
export function LocationFilterES({ activeLocation, onChange }: LocationFilterESProps) {
  ...
  if (loading) {
    return (
      <FilterItem title="UBICACIÓN">
        <div className="text-sm text-fuchsia-300/70 px-3 py-2">Cargando ubicaciones...</div>
      </FilterItem>
    );
  }
  ...
  if (error || cities.length === 0) {
    return (
      <FilterItem title="UBICACIÓN">
        <div className="text-sm text-fuchsia-300/70 px-3 py-2">No hay ubicaciones disponibles</div>
      </FilterItem>
    );
  }
