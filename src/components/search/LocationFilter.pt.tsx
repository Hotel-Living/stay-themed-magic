...
export function LocationFilterPT({ activeLocation, onChange }: LocationFilterPTProps) {
  ...
  if (loading) {
    return (
      <FilterItem title="LOCALIZAÇÃO">
        <div className="text-sm text-fuchsia-300/70 px-3 py-2">Carregando localizações...</div>
      </FilterItem>
    );
  }
  ...
  if (error || cities.length === 0) {
    return (
      <FilterItem title="LOCALIZAÇÃO">
        <div className="text-sm text-fuchsia-300/70 px-3 py-2">Nenhuma localização disponível</div>
      </FilterItem>
    );
  }
