...
export function PropertyStyleFilterPT({ activePropertyStyle, onChange }: PropertyStyleFilterPTProps) {
  ...
  if (loading) {
    return (
      <FilterItem title="ESTILO DA PROPRIEDADE">
        <div className="text-sm text-fuchsia-300/70 px-3 py-2">Carregando estilos de propriedade...</div>
      </FilterItem>
    );
  }
  ...
  if (error || propertyStyles.length === 0) {
    return (
      <FilterItem title="ESTILO DA PROPRIEDADE">
        <div className="text-sm text-fuchsia-300/70 px-3 py-2">Nenhum estilo de propriedade disponível</div>
      </FilterItem>
    );
  }
