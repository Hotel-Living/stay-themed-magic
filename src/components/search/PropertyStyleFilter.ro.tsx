...
export function PropertyStyleFilterRO({ activePropertyStyle, onChange }: PropertyStyleFilterROProps) {
  ...
  if (loading) {
    return (
      <FilterItem title="STILUL PROPRIETĂȚII">
        <div className="text-sm text-fuchsia-300/70 px-3 py-2">Se încarcă stilurile proprietăților...</div>
      </FilterItem>
    );
  }
  ...
  if (error || propertyStyles.length === 0) {
    return (
      <FilterItem title="STILUL PROPRIETĂȚII">
        <div className="text-sm text-fuchsia-300/70 px-3 py-2">Nu sunt stiluri de proprietăți disponibile</div>
      </FilterItem>
    );
  }
