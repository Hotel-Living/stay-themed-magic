...
export function PropertyTypeFilterES({ activePropertyType, onChange }: PropertyTypeFilterESProps) {
  ...
  if (loading) {
    return (
      <FilterItem title="TIPO DE PROPIEDAD">
        <div className="text-sm text-fuchsia-300/70 px-3 py-2">Cargando tipos de propiedad...</div>
      </FilterItem>
    );
  }
  ...
  if (error || propertyTypes.length === 0) {
    return (
      <FilterItem title="TIPO DE PROPIEDAD">
        <div className="text-sm text-fuchsia-300/70 px-3 py-2">No hay tipos de propiedad disponibles</div>
      </FilterItem>
    );
  }
