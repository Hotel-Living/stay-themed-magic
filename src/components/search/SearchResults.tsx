...
export function SearchResults({
  ...
  return <div className="space-y-6 px-2">
      <div className="text-white text-lg font-semibold">
        {hotels.length} hotel{hotels.length !== 1 ? 's' : ''} found
      </div>
      {/* Grid with reduced card width and tighter spacing */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 bg-slate-400">
        {hotels.map(hotel => {
        ...
        // Get main image
        const mainImage = hotel.hotel_images?.find(img => img.is_main)?.image_url || hotel.hotel_images?.[0]?.image_url || hotel.thumbnail || "/placeholder.svg";
        ...
        return <div key={hotel.id} className="w-full max-w-[280px] mx-auto">
              <HotelCard id={hotel.id} name={hotel.name} city={city} country={country} stars={hotel.category || 0} pricePerMonth={hotel.price_per_month} themes={themes} image={mainImage} availableMonths={hotel.availableMonths} rates={hotel.rates} hotel_themes={hotel.hotel_themes} hotel_activities={hotel.hotel_activities} meal_plans={hotel.meal_plans} location={hotel.location} thumbnail={hotel.thumbnail} onClick={() => handleHotelClick(hotel.id)} />
            </div>;
      })}
      </div>
    </div>;
