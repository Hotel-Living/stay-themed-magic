...
export function SecondFilterSidebar({
  ...
  return <div className="bg-gradient-to-br from-purple-950/90 to-purple-900/70 backdrop-blur-md border border-purple-600/30 p-6 shadow-2xl px-0 py-0 rounded-none my-0">
      {/* Reset Button */}
      <div className="mb-6">
        <button onClick={onResetAllFilters} className="w-full py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-colors text-sm font-medium px-0">
      ...
      <div className="mb-6">
        <CheckboxFilter title={t('filters.mealPlan')} options={[t('filters.breakfast'), t('filters.halfBoard'), t('filters.fullBoard'), t('filters.allInclusive')]} selectedOptions={activeFilters.mealPlans} onChange={(value, isChecked) => handleArrayFilterChange('mealPlans', value, isChecked)} />
      </div>
      ...
      <div className="mb-6">
        <CategoryFilter activeCategory={activeFilters.stars?.[0] || null} onChange={value => handleArrayFilterChange('stars', value, true)} />
      </div>

      {/* 12. Room Types Filter - TIPO DE HABITACIÓN */}
      <div className="mb-6">
        <CheckboxFilter title={t('filters.roomTypes')} options={[t('filters.single'), t('filters.double'), t('filters.suite'), t('filters.apartment')]} selectedOptions={activeFilters.roomTypes} onChange={(value, isChecked) => handleArrayFilterChange('roomTypes', value, isChecked)} />
      </div>

      {/* 13. Hotel Features Filter - SERVICIOS DEL HOTEL */}
      <div className="mb-6">
        <CheckboxFilter title={t('filters.hotelFeatures')} options={[t('filters.pool'), t('filters.gym'), t('filters.spa'), t('filters.restaurant'), t('filters.bar'), t('filters.wifi'), t('filters.parking')]} selectedOptions={activeFilters.hotelFeatures} onChange={(value, isChecked) => handleArrayFilterChange('hotelFeatures', value, isChecked)} />
      </div>

      {/* 14. Room Features Filter - SERVICIOS DE LA HABITACIÓN */}
      <div className="mb-6">
        <CheckboxFilter title={t('filters.roomFeatures')} options={[t('filters.airConditioning'), t('filters.balcony'), t('filters.kitchen'), t('filters.workspace'), t('filters.tv'), t('filters.minibar')]} selectedOptions={activeFilters.roomFeatures} onChange={(value, isChecked) => handleArrayFilterChange('roomFeatures', value, isChecked)} />
      </div>
