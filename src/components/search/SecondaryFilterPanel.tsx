...
function CollapsibleSection({
  ...
  return <div className="border-b border-purple-600/30 last:border-b-0">
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between text-white transition-colors bg-[#5a028d] px-0 text-sm font-normal my-0 py-[5px] rounded-sm">
...
export function SecondaryFilterPanel({
  ...
  return <div className="bg-[#2D0A50] rounded-lg overflow-hidden">
      ...
      <CollapsibleSection title={t('filters.mealPlan')}>
        {mealPlanOptions.map(option => <label key={option.key} className="flex items-center space-x-2 text-white text-sm cursor-pointer hover:bg-purple-700/20 p-1 rounded">
