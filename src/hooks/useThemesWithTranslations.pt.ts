...
const fetchThemesPT = async (): Promise<Theme[]> => {
  const { data, error } = await supabase
    ...
    `)
    .eq('theme_translations.locale', 'pt')
...
export const useThemesWithTranslationsPT = () => {
  return useQuery({
    queryKey: ['themes-with-translations', 'pt'],
    queryFn: () => fetchThemesPT(),
    staleTime: 1000 * 60 * 5,
