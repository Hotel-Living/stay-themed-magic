...
const fetchThemesRO = async (): Promise<Theme[]> => {
  const { data, error } = await supabase
    ...
    `)
    .eq('theme_translations.locale', 'ro')
...
export const useThemesWithTranslationsRO = () => {
  return useQuery({
    queryKey: ['themes-with-translations', 'ro'],
    queryFn: () => fetchThemesRO(),
    staleTime: 1000 * 60 * 5,
