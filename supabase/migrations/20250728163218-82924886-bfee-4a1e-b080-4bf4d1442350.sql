-- Test the handle_new_user trigger function with user role
DO $$
DECLARE
    test_user_id UUID := gen_random_uuid();
    test_email TEXT := 'test.user.trigger@example.com';
BEGIN
    -- Simulate what happens when a user signs up with role 'user'
    RAISE NOTICE 'Testing handle_new_user trigger for user role';
