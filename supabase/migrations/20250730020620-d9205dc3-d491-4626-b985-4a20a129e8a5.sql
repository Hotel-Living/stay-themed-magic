-- Update RPC function to remove all auth.uid() dependencies
...
BEGIN
  ...
  VALUES (
    COALESCE((hotel_data->>'owner_id')::UUID, NULL),
    hotel_data->>'name',
  ...
  RAISE LOG 'Hotel registration successful: % for owner: %', new_hotel_id, COALESCE(hotel_data->>'owner_id', 'unknown');
